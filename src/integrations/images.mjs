  ...
  return {
    ...
    hooks: {
      'astro:server:setup': async ({ server, logger }) => {
        ...
        /*
         * Первые секунды после запуска слежение молчит.
         *
         * Перенос уже отработал — он висит на `npm run dev` и выполняется до
         * старта сервера. А наблюдатель, получив папку, сообщает о каждом
         * лежащем в ней файле как о новом, и без этой паузы перенос запускался
         * бы вторично, прямо поверх синхронизации контента. Однажды это
         * закончилось пустым списком изображений и ошибкой «local images must
         * be imported» на постороннем файле.
         */
        let armed = false;
        setTimeout(() => { armed = true; }, 3000);

        const run = async (file) => {
          if (!armed || !file.startsWith(archivePath)) return;
